...
| `wallpaper-set` | Đổi nền random theo giờ (day- 06:00–17:59 / night- còn lại) qua **awww** (transition fade 2s) + **wallust** sinh palette màu theo ảnh → alacritty / waybar / rofi / sway đổi màu theo wallpaper. Ảnh đặt ở `~/Pictures/wallpapers` (tiền tố `day-`/`night-`), thêm ảnh không cần rebuild |
...
| Palette màu không khớp ảnh | Chạy lại `~/.local/bin/wallpaper-set`; file palette ở `~/.config/wallust/colors/`, waybar ở `~/.config/waybar/colors.css` |
